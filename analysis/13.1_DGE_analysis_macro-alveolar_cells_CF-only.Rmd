...
subtitle: "DGE analysis of alveolar macrophages in CF samples only"
...
## Transfer cell labels to raw counts

Although ambient RNA correction can help with clustering and cell labeling, subtracting ambient RNA counts results in *unpredictable statistical properties due the distortion of the mean-variance relationship* when comparing gene expression between conditions within a cell type on pseudobulk samples as per [OSCA](https://bioconductor.org/books/3.13/OSCA.multisample/ambient-problems.html#subtracting-ambient-counts). Therefore, we transfer cell labels from ambient corrected clusters to the raw counts. 

```{r}
file <- here("data",
            "C133_Neeland_merged",
            glue("C133_Neeland_full_clean.SEU.rds"))
...
```{r}
seuRaw <- subset(seuRaw, cells = which(colnames(seuRaw) %in% colnames(seu)))
if(all(colnames(seuRaw) == colnames(seu))){
  seuRaw@meta.data <- seu@meta.data
}

counts <- seuRaw[["RNA"]]@counts
counts <- counts[rownames(counts) %in% rownames(seu),]
seuRaw[["RNA"]]@counts <- counts
dim(seuRaw[["RNA"]]@counts)
```
...
colData(pseudoBulk) %>%
  data.frame %>%
  ggplot(aes(x = sample.id, fill = Micro_code, y = ncells)) +
...
HKGs <- segList$human$human_scSEG
...
for(i in 1:length(dims)){
  ...
  mds <- limma::plotMDS(edgeR::cpm(ySub[ctl, ], 
                                   log = TRUE), 
                        gene.selection = "common",
                        plot = FALSE, dim.plot = dims[[i]])
...
for(k in 1:10){
  ...
  EDASeq::plotPCA(edgeR::cpm(adj$normalizedCounts), 
                  col = cols2[x2], labels = FALSE,
                  pch = 19)
  legend("topleft", fill = cols2, legend = levels(factor(ySub$samples$Disease)))
...
# create the design matrix
design <- model.matrix(~ 0 + group + W + age + sex + micro)
colnames(design)[1:length(levels(group))] <- levels(group)
...
## CF no modifier vs. CF ivacaftor 
...
#### Dotplots summarising top gene sets
...
#### Barcode plots for top gene sets
...
barcodeplot(statistic, index = unlist(h.id[rownames(h.cam)[j]]))
title(main = rownames(h.cam)[j], cex.main = 0.9)
})
```
...
par(mfrow = c(2,2))
sapply(1:4, function(j){
barcodeplot(statistic, index = unlist(c2.id[rownames(c2.cam)[j]]))
title(main = rownames(c2.cam)[j], cex.main = 0.9)
})
```
...
#### Barcode plots for fibrosis GSEA

```{r}
par(mfrow = c(1,2))
sapply(1:2, function(j){
barcodeplot(statistic, index = unlist(f.id[rownames(f.cam)[j]]))
title(main = rownames(f.cam)[j], cex.main = 0.9)
})
```


## CF no modifier severe vs. CF no modifier mild
...
#### Dotplot summarising ORA result
