...
dat %>%
  ...
  tidyHeatmap::heatmap(
    ...
    palette_value = circlize::colorRamp2(seq(0, 2, length.out = 9),
...
ggplot(aes(x = Group, y = Freq, fill = Group)) +
  ...
  scale_fill_paletteer_d("RColorBrewer::Set2", direction = 1) -> group_props
...
img <- readPNG(here("output/Figure1A.png"))
f1a <- rasterGrob(img, interpolate = TRUE)

(wrap_elements(f1a) +
wrap_elements(f1b + annotate("text", x = -Inf, y = Inf, 
           label = paste0(formatC(nrow(seu@meta.data), format = "d", big.mark = ","), " cells"),
           hjust = -0.1, vjust = 1.5, 
           size = 4) + 
...
ggsave(here("output/pdf_figures/Figure_1.pdf"), 
       plot = p, width = 13, height = 15, units = "in", device = cairo_pdf)
```
