...
subtitle: "DGE analysis of alveolar macrophages: CF versus Non-CF controls"
...
<!-- Extract only the untreated CF and control samples. -->

<!-- ```{r} -->
<!-- ySub <- ySub[, !str_detect(ySub$samples$Treatment, "^treated ")] -->
<!-- ``` -->
...
To adjust for gene length bias in ORA, setup the transcript length.
...
<!--   mutate(Batch = as.factor(Batch)) %>% -->
<!--   dplyr::filter(Status_sub %in% names(contr[,i])[abs(contr[,i]) > 0], -->
<!--                 gene %in% deg[deg %in% fibrosis_table$symbol[fibrosis_table$source %in% "pulmonary_fibrosis_ctd"]]) %>% -->
<!--   mutate(Group = ifelse(str_detect(Group, str_remove(grps[1], "CF.")), -->
<!--                         grps[1], -->
<!--                         grps[2])) %>% -->
<!--   ggplot(aes(x = Group, -->
...
<!-- ```{r} -->
<!-- barcodeplot(statistic, index = f.id$pulmonary_fibrosis_ctd) -->
<!-- ``` -->
